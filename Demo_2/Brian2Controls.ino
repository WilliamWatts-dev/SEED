...
void rotate(float angle, long posCounts, bool dir, float speedMultiplier = 1.0) {
  ...
  switch(currentState) {
    ...
    */
    case(initialRotationState): {
        ...
        rotate(20, posL_rad, true, 0.35); // rotates clockwise because of true
        ...
        if(newData == true) {
          ...
          newData = false;
          // Forcibly stop the initial rotation
          stop();
    ...
    case(qrFoundState): {
      ...
      */
      ...
      rotate(abs(readAngle), posL_rad, true, 0.40);
    ...
    case(moveCycleState): {
      ...
      if(abs(readAngle) > 3.0) {

        float correctedAngle = min(abs(readAngle) * 0.7, 10.0);
        
        Serial.print("Correcting angle by: ");
        Serial.print(correctedAngle);
        Serial.println(dir ? " right" : " left");

        rotate(correctedAngle, posL_rad, dir, 0.2);
